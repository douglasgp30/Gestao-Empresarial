...
const testarComissao = () => {
  ...
  const receita = {
    ...
    valor: 500.00,
    valorLiquido: 450.00,
    comissao: 50.00, // ✅ Agora deve ser incluída
    categoria: "Serviços",
...
const testarExclusao = (idParaExcluir) => {
  ...
  try {
    const lancamentosAntes = JSON.parse(localStorage.getItem("lancamentos_caixa") || "[]");
    console.log("   - Lançamentos antes:", lancamentosAntes.length);
    
    // Simular exclusão
    const lancamentosDepois = lancamentosAntes.filter(l => l.id !== idParaExcluir);
...
const testarEdicao = () => {
  ...
  try {
    ...
    const lancamentoEditado = lancamentosAtualizados.find(l => l.id === lancamento.id);
    
    if (lancamentoEditado && lancamentoEditado.valor === 350.00) {
...
const verificarExibicaoComissao = () => {
  ...
  const lancamentos = JSON.parse(localStorage.getItem("lancamentos_caixa") || "[]");
  const receitasComComissao = lancamentos.filter(l => 
    l.tipo === "receita" && l.comissao != null && l.comissao > 0
  );
  
  console.log(`   - Total de receitas com comissão: ${receitasComComissao.length}`);
  
  receitasComComissao.slice(0, 3).forEach((r, i) => {
    console.log(`   ${i + 1}. ID: ${r.id}, Valor: R$ ${r.valor}, Comissão: R$ ${r.comissao}`);
  });
...
console.log("\n🎯 RESULTADO GERAL:", todosTestes ? "✅ TODOS PASSARAM" : "❌ ALGUNS FALHARAM");
...
window.limparDadosTeste = function() {
  ...
  const lancamentosLimpos = lancamentos.filter(l => 
    !l.id.includes("teste-comissao") && 
    !l.id.includes("teste-edicao")
  );
