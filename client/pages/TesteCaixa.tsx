    ...
    const testarAPI = async () => {
      try {
        ...
        const response = await fetch(
          "/api/caixa?dataInicio=2025-08-21&dataFim=2025-08-22",
        );
    ...
    <div className="p-4 max-w-4xl mx-auto space-y-6">
        ...
        {dados?.map((lancamento: any, index: number) => (
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-2 text-sm">
              ...
              <div>
                <strong>Descrição:</strong> {lancamento.descricao.nome}
              </div>
              <div>
                <strong>Forma Pagamento:</strong> {lancamento.formaPagamento}
              </div>
              <div>
                <strong>Funcionário:</strong> {lancamento.funcionario || "N/A"}
              </div>
              <div>
                <strong>Cliente:</strong> {lancamento.cliente || "N/A"}
              </div>
              <div>
                <strong>Comissão:</strong> R$ {lancamento.comissao}
              </div>
              <div>
                <strong>Observações:</strong> {lancamento.observacoes || "N/A"}
              </div>
              <div>
                <strong>Número Nota:</strong> {lancamento.numeroNota || "N/A"}
              </div>
            </div>
      ...
      <div className="mt-6 p-4 bg-blue-50 rounded">
        ...
        <ol className="list-decimal list-inside mt-2 space-y-1 text-sm">
          <li>
            Se as descrições aparecem como "Serviço (corrigido X)" → API
            funcionando ✅
          </li>
