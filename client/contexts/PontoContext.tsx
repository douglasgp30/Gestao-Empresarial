...
export function PontoProvider({ children }: PontoProviderProps) {
  ...
  const [funcionariosComPonto, setFuncionariosComPonto] = useState<
    Funcionario[]
  >([]);
  ...
  const registrarPonto = useCallback(
    async (observacao?: string, vendeuAlmoco?: boolean) => {
      if (!user?.id) return;
      ...
      try {
        ...
        if (isNumericId) {
          ...
          // Usar localStorage para funcionários locais
          resultado = await pontoLocalStorage.registrarPonto(
            user.id,
            observacao,
            vendeuAlmoco,
          );
        ...
        const textoBatida = isNumericId
          ...
          : pontoLocalStorage.obterTextoBatida(
              resultado.batidaRegistrada || "",
            );
  ...
  const carregarHistorico = useCallback(
    async (funcionarioId?: string, pagina: number = 1) => {
  ...
  const carregarFuncionariosComPonto = useCallback(async () => {
    try {
      ...
      const funcionariosLocalStorage =
        await pontoLocalStorage.buscarFuncionariosComPonto();
      ...
      try {
        ...
        console.log(
          "Banco de funcionários não disponível, usando apenas localStorage",
        );
      }

      // Combinar ambas as fontes
      const todosFuncionarios = [
        ...funcionariosLocalStorage,
        ...funcionariosBanco,
      ];
  ...
  const editarPonto = useCallback(
    async (pontoId: string, dados: any) => {
      try {
        ...
        toast({
          ...
          description:
            error.response?.data?.error || "Não foi possível editar o ponto.",
          variant: "destructive",
        });
      }
    },
  ...
  const registrarPontoAdmin = useCallback(
    async (dados: any) => {
      try {
  ...
  const atualizarFiltros = useCallback(
    (novosFiltros: Partial<FiltrosPonto>) => {
      setFiltros((prev) => ({
        ...prev,
        ...novosFiltros,
        __timestamp: Date.now(),
  ...
  return (
    <PontoContext.Provider value={value}>{children}</PontoContext.Provider>
  );
}
