...
import { Agendamento, FiltrosAgendamento, LembreteAgendamento } from '../../shared/types';
...
interface AgendamentosContextType {
  ...
  // CRUD Operations
  criarAgendamento: (agendamento: Omit<Agendamento, 'id' | 'dataCriacao' | 'funcionarioId' | 'status'>) => void;
...
export function AgendamentosProvider({ children }: { children: ReactNode }) {
  ...
  const salvarLembretes = (novosLembretes: LembreteAgendamento[]) => {
    localStorage.setItem('lembretes_agendamentos', JSON.stringify(novosLembretes));
    setLembretes(novosLembretes);
  };

  const criarAgendamento = (dadosAgendamento: Omit<Agendamento, 'id' | 'dataCriacao' | 'funcionarioId' | 'status'>) => {
    if (!user) return;
    ...
    dataHoraLembrete.setMinutes(dataHoraLembrete.getMinutes() - dadosAgendamento.tempoLembrete);
  ...
  const atualizarAgendamento = (id: string, dadosAtualizacao: Partial<Agendamento>) => {
    ...
    if (dadosAtualizacao.dataServico || dadosAtualizacao.horaServico || dadosAtualizacao.tempoLembrete) {
      ...
      if (agendamento) {
        ...
        dataHoraLembrete.setHours(parseInt(hora), parseInt(minutos));
        dataHoraLembrete.setMinutes(dataHoraLembrete.getMinutes() - agendamento.tempoLembrete);
  ...
  const mostrarNotificacaoLembrete = (agendamento: Agendamento) => {
    ...
    const audio = new Audio('data:audio/wav;base64,UklGRnoGAABXQVZFZm10IBAAAAABAAEAQB8AAEAfAAABAAgAZGF0YQoGAACBhYqFbF1fdJivrJBhNjVgodDbq2EcBj+a2/LDciUFLIHO8tiJNwgZaLvt559NEAxQp+PwtmMcBjiR1/LMeSwFJHfH8N2QQAoUXrTp66hVFApGn+DyvmYZBjuY3PDCeyw=');
    audio.volume = 0.3;
    ...
    window.dispatchEvent(new CustomEvent('mostrarLembreteAgendamento', {
      detail: agendamento
    }));
  };
  ...
  const agendamentosFiltrados = agendamentos.filter(ag => {
    ...
    const statusMatch = filtros.status === "todos" || ag.status === filtros.status;
    const setorMatch = !filtros.setor || ag.setor === filtros.setor;
    const tecnicoMatch = !filtros.tecnico || ag.tecnicoResponsavel === filtros.tecnico;
  ...
  const proximosLembretes = lembretes.filter(l => !l.lido && l.dataHoraLembrete <= new Date());
