...
function calcularAtraso(
  ...
  const entrada =
    horaEntrada instanceof Date ? horaEntrada : new Date(horaEntrada);
  ...
  return Math.floor(
    (entrada.getTime() - inicioExpediente.getTime()) / (1000 * 60),
  );
}
...
export class PontoLocalStorageService {
  ...
  async registrarPonto(
    ...
    let pontoIndex = pontos.findIndex(
      (p) =>
        p.funcionarioId === funcionarioId &&
        new Date(p.data).getTime() === hoje.getTime(),
    );
    ...
    if (proximaBatida === "saida") {
      ...
      ponto.horasExtras = calcularHorasExtras(
        ponto.totalHoras || 0,
        jornadaDiaria,
      );
      ponto.saldoHoras = calcularSaldoHoras(
        ponto.totalHoras || 0,
        jornadaDiaria,
      );
    }
  ...
  async buscarFuncionariosComPonto(): Promise<Funcionario[]> {
    try {
      ...
      return funcionarios.filter(
        (f: Funcionario) => f.registraPonto && f.ativo,
      );
    } catch (error) {
  ...
  async gerarRelatorio(
    ...
    // Buscar funcionário
    const funcionariosStorage = localStorage.getItem("funcionarios");
    const funcionarios = funcionariosStorage
      ? JSON.parse(funcionariosStorage)
      : [];
    const funcionario = funcionarios.find(
    ...
    const pontos = await this.buscarHistoricoPonto(
      funcionarioId,
      dataInicio,
      dataFim,
    );
