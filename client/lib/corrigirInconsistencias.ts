...
export function corrigirInconsistenciasFormasPagamento() {
  try {
    console.log('[CorrigirInconsistencias] Iniciando correção das formas de pagamento...');
    ...
    if (formasExistentes) {
      ...
      if (forma5 && !forma5.nome.toLowerCase().includes('boleto')) {
        console.log('[CorrigirInconsistencias] Inconsistência detectada no ID 5, corrigindo...');
        
        // Substituir por dados corretos
        localStorage.setItem('formas_pagamento', JSON.stringify(FORMAS_PAGAMENTO_PADRAO));
        
        console.log('[CorrigirInconsistencias] Formas de pagamento corrigidas com sucesso');
        return true;
      }
    } else {
    ...
    console.error('[CorrigirInconsistencias] Erro ao corrigir inconsistências:', error);
    return false;
  }
}
...
export function corrigirInconsistenciasClientes() {
  try {
    ...
    if (clientesExistentes) {
      ...
      console.log('[CorrigirInconsistencias] Clientes verificados e corrigidos se necessário');
    }
    
    return true;
  } catch (error) {
    console.error('[CorrigirInconsistencias] Erro ao corrigir clientes:', error);
    return false;
  }
}
...
export function executarTodasCorrecoes() {
  ...
  if (resultadoFormas || resultadoClientes) {
    console.log('[CorrigirInconsistencias] Correções aplicadas, recarregue a página para ver os efeitos');
    return true;
  }
