...
class PontoApiService {
  ...
  async buscarPontoHoje(funcionarioId: string): Promise<PontoDoFuncionario> {
    try {
      const response = await apiService.get(
        `/ponto/funcionario/${funcionarioId}/hoje`,
      );
  ...
  async buscarTodosPontos(
    filtros: BuscarTodosPontosRequest = {},
  ): Promise<Ponto[]> {
    try {
      const params = new URLSearchParams();
      ...
      if (filtros.funcionarioId)
        params.append("funcionarioId", filtros.funcionarioId);
      if (filtros.status) params.append("status", filtros.status);

      const response = await apiService.get(
        `/ponto/todos?${params.toString()}`,
      );
      return response.data?.data || response.data;
    } catch (error) {
