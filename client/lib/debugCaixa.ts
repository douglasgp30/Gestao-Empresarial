      ...
      lancamentos.forEach((lancamento: any, index: number) => {
        ...
        if (
          ...
          problemasEncontrados.push(
            `Lançamento ${index + 1}: descrição é código numérico (${lancamento.descricao})`,
          );
        }
        ...
        if (
          ...
          /^\d+$/.test(lancamento.formaPagamento)
        ) {
          problemasEncontrados.push(
            `Lançamento ${index + 1}: forma de pagamento é código numérico (${lancamento.formaPagamento})`,
          );
        }
        ...
        if (
          ...
          problemasEncontrados.push(
            `Lançamento ${index + 1}: técnico é código numérico (${lancamento.tecnicoResponsavel})`,
          );
        }
        ...
        if (
          ...
          problemasEncontrados.push(
            `Lançamento ${index + 1}: cliente é código numérico (${lancamento.cliente})`,
          );
        }
      });
  ...
  try {
    ...
    const lancamentosMigrados = lancamentos.map((lancamento: any) => {
      return {
          ...
          /^\d+$/.test(lancamento.formaPagamento)
            ...
            : typeof lancamento.formaPagamento === "string"
              ? {
                  ...
                  nome: lancamento.formaPagamento,
                }
        ...
        // Corrigir setor
        setor:
          typeof lancamento.setor === "string" && /^\d+$/.test(lancamento.setor)
