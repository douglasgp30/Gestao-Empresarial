...
function migrateLancamento(lancamento: any): any {
  ...
  if (typeof migrado.formaPagamento === "string") {
    console.log(`  💳 Migrando formaPagamento: "${migrado.formaPagamento}" -> objeto`);
...
export function executarMigracaoCompleta(): MigrationResult {
  ...
  try {
    ...
    console.log(`📊 [Migração] ${lancamentosOriginais.length} lançamentos para migrar`);

    // Migrar cada lançamento
    const lancamentosMigrados = lancamentosOriginais.map((lancamento, index) => {
      try {
        console.log(`\n🔄 [Migração] Processando lançamento ${index + 1}/${lancamentosOriginais.length}`);
        return migrateLancamento(lancamento);
      } catch (error) {
        console.error(`❌ [Migração] Erro ao migrar lançamento ${index + 1}:`, error);
    ...
    console.log("🔄 [Migração] Recarregue a página para ver os dados atualizados");
...
if (typeof window !== 'undefined') {
  ...
  console.log("  - window.migracaoCaixa.executar() - Executa migração completa");
  console.log("  - window.migracaoCaixa.verificar() - Verifica se migração é necessária");
