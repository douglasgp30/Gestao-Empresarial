...
export const migrarLancamentosParaBanco = async () => {
  try {
    console.log(
      "🔄 Iniciando migração de lançamentos para o banco de dados...",
    );
    ...
    console.log(
      `📊 Encontrados ${lancamentosLocal.length} lançamentos no localStorage`,
    );
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (dadosBanco.length > 0) {
          console.log(
            `⚠️ Já existem ${dadosBanco.length} lançamentos no banco de dados`,
          );
      ...
      console.log(
        "📡 Erro ao verificar dados do banco, continuando com migração...",
      );
    }
    ...
    for (let i = 0; i < lancamentosLocal.length; i++) {
      ...
      try {
        console.log(
          `📤 Migrando lançamento ${i + 1}/${lancamentosLocal.length}:`,
          lancamento.id,
        );
        ...
        const dadosParaAPI = {
          ...
          formaPagamentoId: extrairId(
            lancamento.formaPagamento,
            lancamento.formaPagamentoId,
          ),
          funcionarioId: extrairId(
            lancamento.tecnicoResponsavel,
            lancamento.tecnicoResponsavelId,
          ),
        ...
        if (response.ok) {
          ...
          console.log(
            `✅ Lançamento ${i + 1} migrado com sucesso:`,
            resultado.id,
          );
          sucessos++;
        } else {
...
export const migrarUmLancamento = async (indice: number = 0) => {
  try {
    ...
    if (indice >= lancamentosLocal.length) {
      throw new Error(
        `Índice ${indice} não existe. Total: ${lancamentosLocal.length}`,
      );
    }
