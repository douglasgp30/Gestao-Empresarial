...
export const limparLocalStorageCaixa = () => {
  ...
  chavesParaLimpar.forEach((chave) => {
    ...
    if (valorAntigo) {
      try {
        ...
        console.log(
          `📦 [LimparStorage] Removendo ${chave}: ${Array.isArray(dados) ? dados.length : 1} item(s)`,
        );
      } catch (e) {
        console.log(
          `📦 [LimparStorage] Removendo ${chave}: ${valorAntigo.length} caracteres`,
        );
      }
  ...
  console.log(
    `✅ [LimparStorage] Limpeza concluída! ${itensLimpos} chaves removidas.`,
  );
  console.log(
    "🔄 [LimparStorage] Recarregue a página para que os contextos sejam reinicializados.",
  );
...
if (typeof window !== "undefined") {
  ...
  console.log(
    "🔧 [LimparStorage] Função disponível globalmente: window.limparLocalStorageCaixa()",
  );
}
