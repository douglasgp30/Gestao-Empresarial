...
class GlobalLoadingControl {
  ...
  private checkHotReload() {
    ...
    this.isHotReloading = url.includes('reload=') || url.includes('?t=') || url.includes('&t=');
    
    if (this.isHotReloading) {
      console.log('🔥 [GlobalLoadingControl] Hot reload detectado, bloqueando carregamentos por 10s');
  ...
  shouldSkipLoading(contextName: string = 'Unknown'): boolean {
    if (this.loadingBlocked) {
      console.log(`⏸️ [GlobalLoadingControl] Bloqueando carregamento para ${contextName} (hot reload em progresso)`);
      return true;
    }
    return false;
  }
  ...
  forceUnblock() {
    ...
    console.log('🔓 [GlobalLoadingControl] Carregamentos forçadamente desbloqueados');
  }
}
