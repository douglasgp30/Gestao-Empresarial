...
export async function testCadastroDinamico(): Promise<TestResult[]> {
  ...
  try {
    ...
    const entidadesContext = document.querySelector('[data-context="entidades"]');
    results.push({
      category: 'Cadastro Dinâmico',
      test: 'Contexto de Entidades',
      status: entidadesContext ? 'pass' : 'fail',
      message: entidadesContext ? 'Contexto carregado com sucesso' : 'Contexto não encontrado'
    });
    ...
    results.push({
      ...
      message: setorSelect ? 'Seletor de setores encontrado' : 'Seletor de setores não encontrado na página atual'
    });
...
export async function testMetaMensal(): Promise<TestResult[]> {
  ...
  try {
    ...
    results.push({
      ...
      message: metaArmazenada ? `Meta armazenada: R$ ${metaArmazenada}` : 'Meta não definida para este mês'
    });
...
export async function testFiltrosData(): Promise<TestResult[]> {
  ...
  try {
    ...
    const mesAtualButton = document.querySelector('button[data-filtro="mesAtual"]');
    results.push({
      category: 'Filtros de Data',
      test: 'Filtro Este Mês',
      status: mesAtualButton ? 'pass' : 'warning',
      message: mesAtualButton ? 'Botão "Este Mês" encontrado' : 'Botão "Este Mês" não encontrado'
    });
...
export async function testAPI(): Promise<TestResult[]> {
  ...
  try {
    ...
    results.push({
      ...
      message: response.ok ? 'API respondendo normalmente' : `Erro na API: ${response.status}`
    });
    
    // Teste de dados de setores
    const setoresResponse = await fetch('/api/setores');
    const setoresData = setoresResponse.ok ? await setoresResponse.json() : null;
    results.push({
      category: 'API',
      test: 'Dados de Setores',
      status: setoresData && Array.isArray(setoresData) ? 'pass' : 'fail',
      message: setoresData ? `${setoresData.length} setores carregados` : 'Erro ao carregar setores'
    });
