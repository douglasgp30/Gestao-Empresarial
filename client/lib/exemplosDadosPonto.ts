...
export function adicionarExemplosFuncionarios(): void {
  try {
    ...
    let funcionarios = funcionariosExistentes
      ? JSON.parse(funcionariosExistentes)
      : [];

    // Verificar se os exemplos já existem
    const idsExistentes = funcionarios.map((f: any) => f.id);
    const novosExemplos = exemplosFuncionarios.filter(
      (f) => !idsExistentes.includes(f.id),
    );
    ...
    if (novosExemplos.length > 0) {
      ...
      console.log(
        `${novosExemplos.length} funcionários de exemplo adicionados ao localStorage`,
      );
    } else {
