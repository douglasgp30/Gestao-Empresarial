...
export const nativeFetch = async (url: string, options?: RequestInit): Promise<Response> => {
  try {
    // Usar fetch original
...
export const tryRestoreNativeFetch = (): void => {
  if (isFetchIntercepted()) {
    console.log('[NativeFetch] Fetch interceptado detectado, tentando usar versão original');
