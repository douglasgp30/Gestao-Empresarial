...
interface FiltroDataSimplesProps {
  label: string;
  dataInicio: Date;
  dataFim: Date;
...
export default function FiltroDataSimples({
  ...
  const aplicarPeriodo = (tipo: string) => {
    ...
    // Aplicar através das props callbacks
  ...
  const dataInicioSegura = dataInicio || new Date();
  const dataFimSegura = dataFim || new Date();
  
  const periodoAtual = `${formatarDataDisplay(dataInicioSegura)} - ${formatarDataDisplay(dataFimSegura)}`;
              ...
              <div className="border-t pt-2 mt-2">
                ...
                <div className="grid grid-cols-2 gap-2 mt-1">
                  <div>
                    <Input
                      type="date"
                      value={formatDateForInput(dataInicioSegura)}
                      onChange={(e) => {
