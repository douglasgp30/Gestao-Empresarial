...
export function PasswordStrength({
  ...
  const criteriosAtendidos = criterios.filter((criterio) =>
    criterio.test(password),
  );
  const pontuacao = criteriosAtendidos.reduce(
    (total, criterio) => total + criterio.weight,
    0,
  );
  const pontuacaoMaxima = criterios.reduce(
    (total, criterio) => total + criterio.weight,
    0,
  );
  ...
  const getNivelForca = () => {
    if (password.length >= 6)
      return { nivel: "Válida", cor: "bg-green-500", texto: "text-green-700" };
...
export function validarForcaSenha(password: string): {
  valida: boolean;
  pontuacao: number;
} {
