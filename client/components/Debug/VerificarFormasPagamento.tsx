...
export function VerificarFormasPagamento() {
  ...
  const statusGeral = !duplicatasAPI && !duplicatasContexto && formasAPI.length > 0 && formasPagamento.length > 0;
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Card>
            <CardContent className="p-4">
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm font-medium">API (/api/formas-pagamento)</span>
                <div className="flex gap-1">
                  <Badge variant={duplicatasAPI ? "destructive" : "default"}>
                    {formasAPI.length} itens
                  </Badge>
                  {duplicatasAPI && <Badge variant="destructive">Duplicatas</Badge>}
                </div>
              </div>
              <div className="text-xs text-gray-600">
                {formasAPI.map(f => f.nome).join(", ") || "Nenhuma forma carregada"}
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardContent className="p-4">
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm font-medium">Contexto (useEntidades)</span>
                <div className="flex gap-1">
                  <Badge variant={duplicatasContexto ? "destructive" : "default"}>
                    {formasPagamento?.length || 0} itens
                  </Badge>
                  {duplicatasContexto && <Badge variant="destructive">Duplicatas</Badge>}
                </div>
              </div>
              <div className="text-xs text-gray-600">
                {formasPagamento?.map(f => f.nome).join(", ") || "Nenhuma forma carregada"}
              </div>
            </CardContent>
          </Card>
        </div>
        ...
        {(duplicatasAPI || duplicatasContexto) && (
          <div className="space-y-2">
            <h4 className="font-medium text-red-800">Detalhes dos Problemas:</h4>
            
            {duplicatasAPI && (
              <div className="p-2 bg-red-50 border border-red-200 rounded">
                <div className="text-sm text-red-700">
                  <strong>API tem duplicatas:</strong> Múltiplos registros com o mesmo nome.
                </div>
              </div>
            )}
            ...
            {duplicatasContexto && (
              <div className="p-2 bg-red-50 border border-red-200 rounded">
                <div className="text-sm text-red-700">
                  <strong>Contexto tem duplicatas:</strong> Lista local com nomes repetidos.
                </div>
              </div>
            )}
          </div>
        ...
        {statusGeral && (
          <div className="p-3 bg-green-50 border border-green-200 rounded">
            ...
            <div className="text-sm text-green-700 space-y-1">
              ...
              <div>• Contexto carregou {formasPagamento?.length} formas únicas</div>
