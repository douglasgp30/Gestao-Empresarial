...
import { RefreshCw, Eye, EyeOff, Trash2, CheckCircle, XCircle } from "lucide-react";
...
export function DebugPrimeiroAcesso() {
  ...
  const loadLocalStorageData = () => {
    ...
    if (funcionariosStorage) {
      try {
        ...
        administradores = funcionarios.filter((f: any) => 
  ...
  const resetarCompleto = () => {
    const chaves = ['funcionarios', 'auth_user', 'primeiro_acesso_completo', 'tour_visualizado', 'tour_completo'];
    chaves.forEach(chave => localStorage.removeItem(chave));
    toast.info("Todas as chaves removidas. Sistema voltará ao primeiro acesso.");
    loadLocalStorageData();
  };
        ...
        <CardHeader>
          <CardTitle className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              ...
              <Button variant="outline" size="sm" onClick={loadLocalStorageData}>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="p-4 border rounded-lg">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                Funcionários: <Badge variant="secondary">
                  {localStorageData._verificacao?.quantidadeFuncionarios || 0}
                </Badge>
              </div>
              <div>
                Administradores: <Badge variant="secondary">
                  {localStorageData._verificacao?.quantidadeAdministradores || 0}
                </Badge>
              </div>
            </div>
            ...
            {localStorageData._verificacao?.administradores?.length > 0 && (
              <div className="mt-3">
                <h4 className="text-sm font-medium mb-2">Administradores encontrados:</h4>
                {localStorageData._verificacao.administradores.map((admin: any, index: number) => (
                  <div key={index} className="text-sm p-2 bg-muted rounded">
                    <strong>{admin.nomeCompleto}</strong> ({admin.login}) - 
                    Ativo: {admin.ativo ? "✅" : "❌"}, 
                    Permissão: {admin.permissaoAcesso ? "✅" : "❌"},
                    Tipo: {admin.tipoAcesso}
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="space-y-3">
            ...
            {Object.entries(localStorageData).map(([chave, dados]: [string, any]) => {
              ...
              return (
                <div key={chave} className="flex items-center justify-between p-3 border rounded">
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <strong>{chave}</strong>
                      <Badge variant={dados?.existe ? "default" : "destructive"}>
