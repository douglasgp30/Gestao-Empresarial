...
export default function FiltrosRelatorios() {
  ...
  const [tempDataInicio, setTempDataInicio] = useState<Date | undefined>(filtros.periodo.dataInicio);
  const [tempDataFim, setTempDataFim] = useState<Date | undefined>(filtros.periodo.dataFim);
  ...
  const isPeriodoAtivo = (tipo: typeof opcoesFiltro[number]['id']): boolean => {
    ...
    switch (tipo) {
      case 'mesAtual': {
        ...
        return filtros.periodo.dataInicio.toDateString() === inicioMes.toDateString() &&
      ...
      case 'ultimos7dias': {
        ...
        return filtros.periodo.dataInicio.toDateString() === inicio7dias.toDateString() &&
               filtros.periodo.dataFim.toDateString() === hoje.toDateString();
      }
      case 'estaemana': {
        const diaSemana = hoje.getDay();
        ...
        return filtros.periodo.dataInicio.toDateString() === inicioSemana.toDateString() &&
               filtros.periodo.dataFim.toDateString() === hoje.toDateString();
      }
      case 'ultimos30dias': {
        const inicio30dias = new Date(hoje.getFullYear(), hoje.getMonth(), hoje.getDate() - 29);
        return filtros.periodo.dataInicio.toDateString() === inicio30dias.toDateString() &&
               filtros.periodo.dataFim.toDateString() === hoje.toDateString();
      }
      default:
        return false;
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Período selecionado: {formatDate(filtros.periodo.dataInicio)} - {formatDate(filtros.periodo.dataFim)}
        </CardDescription>
      </CardHeader>
          ...
          <div className="space-y-2">
            ...
            <Select 
              ...
              onValueChange={(value) => setFiltros({ 
                ...filtros, 
                tipo: value === 'todos' ? undefined : value as 'receitas' | 'despesas'
              })}
            >
              <SelectTrigger>
                <SelectValue />
