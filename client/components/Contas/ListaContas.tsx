...
export default function ListaContas() {
  ...
  const contasFiltradas = contas.filter(conta => {
    ...
    const tipoCorreto = filtros.tipo === 'ambos' || conta.tipo === filtros.tipo;
    const statusCorreto = !filtros.status || conta.status === filtros.status;
    const fornecedorCorreto = !filtros.fornecedorCliente || 
      conta.fornecedorCliente.toLowerCase().includes(filtros.fornecedorCliente.toLowerCase());

    return dentroDataInicio && dentroDataFim && tipoCorreto && statusCorreto && fornecedorCorreto;
  }).sort((a, b) => {
              ...
              <TableBody>
                {contasFiltradas.map((conta) => (
                  <TableRow key={conta.id}>
                    ...
                    <TableCell>
                      <Badge 
                        variant={conta.tipo === 'receber' ? 'default' : 'destructive'}
                      ...
                      >
                        ...
                        <span>{conta.tipo === 'receber' ? 'A Receber' : 'A Pagar'}</span>
                      </Badge>
                    </TableCell>
                    ...
                    <TableCell>
                      <Badge className={`flex items-center space-x-1 w-fit ${getStatusColor(conta.status)}`}>
      ...
      <AlertDialog open={!!contaParaPagar} onOpenChange={() => setContaParaPagar(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Tem certeza que deseja marcar esta conta como paga? Esta ação registrará a data de pagamento como hoje.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancelar</AlertDialogCancel>
            <AlertDialogAction
              onClick={() => contaParaPagar && handleMarcarComoPaga(contaParaPagar)}
      ...
      <AlertDialog open={!!contaParaExcluir} onOpenChange={() => setContaParaExcluir(null)}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel>Cancelar</AlertDialogCancel>
            <AlertDialogAction
              onClick={() => contaParaExcluir && handleExcluir(contaParaExcluir)}
