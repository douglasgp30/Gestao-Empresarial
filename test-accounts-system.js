...
async function testAccountsSystem() {
  ...
  try {
    ...
    console.log(
      ...
      fornecedoresData.data?.length || 0,
    );
    ...
    const contasResponse = await fetch(
      `${baseURL}/contas?dataInicio=${hoje}&dataFim=${dataFimStr}`,
    );
    ...
    if (contasData.data?.length > 0) {
      ...
      contasData.data.slice(0, 3).forEach((conta, index) => {
        console.log(
          `   ${index + 1}. ${conta.tipo.toUpperCase()} - ${conta.cliente?.nome || conta.fornecedor?.nome} - R$ ${conta.valor} - Vence: ${new Date(conta.dataVencimento).toLocaleDateString("pt-BR")}`,
        );
      });
    }
    ...
    if (clientesData.data?.length > 0 && categoriasData.data?.length > 0) {
      ...
      const novaContaReceber = {
        ...
        descricaoCategoria: categoriasData.data.find(
          (c) => c.tipo === "receita",
        )?.id,
        pago: false,
      };
      ...
      if (criarContaResponse.ok) {
        ...
        console.log(
          `   ID: ${contaCriada.data.codLancamentoContas}, Valor: R$ ${contaCriada.data.valor}`,
        );
      } else {
    ...
    const totaisResponse = await fetch(
      `${baseURL}/contas/totais?dataInicio=${hoje}&dataFim=${dataFimStr}`,
    );
    ...
    if (totaisResponse.ok && totaisData.data) {
      ...
      console.log(
        `   Total a Receber: R$ ${totaisData.data.totalReceber.toFixed(2)}`,
      );
      console.log(
        `   Total a Pagar: R$ ${totaisData.data.totalPagar.toFixed(2)}`,
      );
      console.log(
        `   Vencendo Hoje: R$ ${totaisData.data.totalVencendoHoje.toFixed(2)}`,
      );
      console.log(
        `   Atrasadas: R$ ${totaisData.data.totalAtrasadas.toFixed(2)}`,
      );
    }
