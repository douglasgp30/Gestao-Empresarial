...
export const cleanDuplicateFormasPagamento: RequestHandler = async (
  req,
  res,
) => {
  try {
    ...
    const grupos = todasFormas.reduce(
      (acc, forma) => {
    ...
    for (const [nome, formas] of Object.entries(grupos)) {
      if (formas.length > 1) {
        console.log(
          `🔍 Encontradas ${formas.length} duplicatas para: "${nome}"`,
        );
        ...
        paraRemover.forEach((forma) => {
          ...
          console.log(
            `   ❌ Marcando para remoção: ID ${forma.id} - "${forma.nome}"`,
          );
        });
