...
export async function cleanDuplicateFuncionarios() {
  try {
    console.log(
      "[CleanDuplicates] Iniciando limpeza de funcionários duplicados...",
    );
    ...
    console.log(
      `[CleanDuplicates] Encontrados ${funcionarios.length} funcionários`,
    );
    ...
    for (const [nome, group] of funcionariosGrouped) {
      if (group.length > 1) {
        console.log(
          `[CleanDuplicates] Encontradas ${group.length} duplicatas para "${nome}"`,
        );

        // Manter o primeiro (mais antigo)
        const [keep, ...toRemove] = group;
        console.log(
          `[CleanDuplicates] Mantendo ID ${keep.id}, removendo IDs: ${toRemove.map((f) => f.id).join(", ")}`,
        );
        ...
        for (const duplicate of toRemove) {
          try {
            ...
            duplicatesRemoved++;
            console.log(
              `[CleanDuplicates] Removido funcionário ID ${duplicate.id} - "${duplicate.nome}"`,
            );
          } catch (error) {
            console.error(
              `[CleanDuplicates] Erro ao remover funcionário ID ${duplicate.id}:`,
              error,
            );
          }
        }
    ...
    console.log(
      `[CleanDuplicates] Limpeza concluída. Removidos ${duplicatesRemoved} funcionários duplicados.`,
    );
