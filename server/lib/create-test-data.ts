...
async function createTestData() {
  ...
  try {
    ...
    const categories = await prisma.descricaoECategoria.count({ where: { tipoItem: 'categoria' } });
    const descriptions = await prisma.descricaoECategoria.count({ where: { tipoItem: 'descricao' } });
    
    console.log(`📊 Total items: ${totalItems} (${categories} categories, ${descriptions} descriptions)`);
