...
export async function seedBasicData() {
  ...
  try {
    ...
    const campanhas = [{ nome: "Campanha Padrão" }, { nome: "Promoção Verão" }];
    ...
    console.log(
      ...
      formasCreated.map((f) => ({ id: f.id, nome: f.nome })),
    );
    console.log(
      "👥 Funcionários criados:",
      funcionariosCreated.map((f) => ({ id: f.id, nome: f.nome })),
    );
  } catch (error) {
