...
async function testAndCreateData() {
  try {
    ...
    if (getData.data && getData.data.length > 0) {
      ...
      const categorias = getData.data.filter(item => item.tipoItem === 'categoria');
      const descricoes = getData.data.filter(item => item.tipoItem === 'descricao');
      
      console.log('\n📂 Categorias:');
      categorias.forEach(cat => console.log(`  - ${cat.nome} (${cat.tipo})`));
      
      console.log('\n📝 Descrições:');
      descricoes.forEach(desc => console.log(`  - ${desc.nome} (${desc.categoria})`));
    ...
    const categoriaResponse = await fetch(`${API_BASE}/api/descricoes-e-categorias`, {
      method: 'POST',
    ...
    const produtosResponse = await fetch(`${API_BASE}/api/descricoes-e-categorias`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        nome: "Produtos",
    ...
    for (const desc of descricoes) {
      ...
      console.log(`  Status: ${response.status}`, result.data?.nome || result.error);
    }
    
    // 5. Verificar dados finais
    console.log('\n5. Verificando dados após criação...');
    const finalResponse = await fetch(`${API_BASE}/api/descricoes-e-categorias`);
    const finalData = await finalResponse.json();
    
    if (finalData.data) {
      const categorias = finalData.data.filter(item => item.tipoItem === 'categoria');
      const descricoes = finalData.data.filter(item => item.tipoItem === 'descricao');
      ...
      console.log('\n📂 Categorias criadas:');
      categorias.forEach(cat => console.log(`  - ${cat.nome} (ID: ${cat.id})`));
