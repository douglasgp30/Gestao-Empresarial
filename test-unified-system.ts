...
async function runTests() {
  ...
  try {
    ...
    const activeItems = await prisma.descricaoECategoria.count({ where: { ativo: true } });
    const categories = await prisma.descricaoECategoria.count({ where: { tipoItem: 'categoria', ativo: true } });
    const descriptions = await prisma.descricaoECategoria.count({ where: { tipoItem: 'descricao', ativo: true } });
    
    console.log(`   ✅ Total items: ${totalItems} (${activeItems} active)`);
    console.log(`   ✅ Categories: ${categories}, Descriptions: ${descriptions}\n`);
    ...
    console.log(`   ✅ Receipt categories: ${receiptCategories.length} (${receiptCategories.map(c => c.nome).join(', ')})`);
    console.log(`   ✅ Expense categories: ${expenseCategories.length} (${expenseCategories.map(c => c.nome).join(', ')})\n`);
    ...
    for (const category of [...receiptCategories, ...expenseCategories]) {
      ...
      console.log(`   ✅ ${category.nome} (${category.tipo}): ${descriptions.length} descriptions`);
    ...
    console.log(`   ✅ API: Receipt categories endpoint would return: ${receiptCategoriesAPI.length} items`);
    console.log(`   ✅ API: Service descriptions endpoint would return: ${serviceDescriptions.length} items\n`);
    ...
    console.log(`   ✅ Created new description: ${newDescription.nome} (under ${newDescription.categoria})`);
    ...
    console.log(`   ✅ Query verification: Found ${marketingDescriptions.length} descriptions under Marketing\n`);
    ...
    console.log(`   ✅ Legacy compatibility: ${oldCategories} old categories, ${oldDescriptions} old descriptions`);
    console.log(`   ✅ New unified table contains all data and is ready for production\n`);

    console.log('🎉 All tests passed! The unified system is working correctly.\n');
    
    // Final summary
    const finalCount = await prisma.descricaoECategoria.count({ where: { ativo: true } });
    const finalCategories = await prisma.descricaoECategoria.count({ where: { tipoItem: 'categoria', ativo: true } });
    const finalDescriptions = await prisma.descricaoECategoria.count({ where: { tipoItem: 'descricao', ativo: true } });
