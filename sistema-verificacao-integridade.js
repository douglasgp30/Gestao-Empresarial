...
const IntegrityChecker = {
    ...
    try {
      ...
      lancamentos.forEach((l, index) => {
        ...
        if (typeof l.valor !== "number")
          problemas.push(`Lançamento ${index}: Valor não é número`);
        if (l.comissao && typeof l.comissao !== "number")
          problemas.push(`Lançamento ${index}: Comissão não é número`);
  ...
  verificarComissoes() {
    ...
    try {
      ...
      const receitasComComissao = receitas.filter(
        (l) => l.comissao != null && l.comissao > 0,
      );
  ...
  verificarFiltros() {
    ...
    console.log(
      `   - Lançamentos no período: ${lancamentosMes.length} de ${lancamentos.length}`,
    );
  ...
  testarAdicionar() {
    try {
      ...
      const lancamentosDepois = JSON.parse(
        localStorage.getItem("lancamentos_caixa") || "[]",
      ).length;

      console.log(
        `     - Adicionar: ${lancamentosAntes} → ${lancamentosDepois}`,
      );
  ...
  testarExcluir() {
    try {
      ...
      const lancamentoTeste = lancamentos.find(
        (l) => l.id && l.id.includes("teste-integridade"),
      );
      if (!lancamentoTeste) {
      ...
      const lancamentosDepois = lancamentos.filter(
        (l) => l.id !== lancamentoTeste.id,
      );
      localStorage.setItem(
        "lancamentos_caixa",
      ...
      const lancamentosFinal = JSON.parse(
        localStorage.getItem("lancamentos_caixa") || "[]",
      ).length;
...
console.log(
  "💡 Use 'IntegrityChecker.verificarComissoes()' para verificar apenas comissões",
);
