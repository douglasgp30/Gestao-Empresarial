...
const fetchCaixaData = async () => {
  try {
    ...
    const response = await fetch('http://localhost:8080/api/caixa?dataInicio=2025-08-21&dataFim=2025-08-22');
    ...
    data.forEach((lancamento, index) => {
      ...
      console.log('Descrição completa:', JSON.stringify({
        descricao: lancamento.descricao,
        descricaoECategoria: lancamento.descricaoECategoria
      }, null, 2));
      
      // Verificar outros campos importantes
      console.log('FormaPagamento:', JSON.stringify(lancamento.formaPagamento, null, 2));
      console.log('Funcionario:', JSON.stringify(lancamento.funcionario, null, 2));
